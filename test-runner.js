import { ethers } from 'ethers';
import { readFileSync } from 'fs';
import { dirname, join } from 'path';
import { fileURLToPath } from 'url';

// Load environment variables
import dotenv from 'dotenv';
dotenv.config();

// Get the directory name of the current module
const __dirname = dirname(fileURLToPath(import.meta.url));

async function runTests() {
  console.log('Running tests with the provided RPC and private key...\n');

  // Initialize provider for Avalanche
  const provider = new ethers.JsonRpcProvider(
    process.env.AVALANCHE_FUJI_RPC_URL || process.env.AVALANCHE_RPC_URL
  );

  // Initialize wallet with the private key
  const wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);

  console.log('Connected to Avalanche network');
  console.log('Wallet address:', wallet.address);

  // Get wallet balance
  const balance = await provider.getBalance(wallet.address);
  console.log('Wallet balance:', ethers.formatEther(balance), 'AVAX\n');

  // Read the compiled contract bytecode
  const contractPath = join(__dirname, 'out', 'SimpleStorage.sol', 'SimpleStorage.json');
  let contractArtifact;
  
  try {
    const contractData = readFileSync(contractPath, 'utf8');
    contractArtifact = JSON.parse(contractData);
  } catch (error) {
    console.log('Compiled contract not found, using source code instead...\n');
    
    // Define the contract ABI and bytecode for SimpleStorage
    const simpleStorageABI = [
      "function set(uint256 x) public",
      "function get() public view returns (uint256)",
      "function storedData() public view returns (uint256)"
    ];
    
    // For testing purposes, we'll deploy a new instance
    console.log('Testing SimpleStorage contract deployment and interaction...\n');
    
    // Get the contract factory
    const contractFactory = new ethers.ContractFactory(
      simpleStorageABI,
      // Placeholder bytecode - in a real scenario, this would come from compilation
      "",
      wallet
    );

    // Deploy the contract
    console.log('Deploying SimpleStorage contract...');
    const simpleStorage = await contractFactory.deploy();
    await simpleStorage.waitForDeployment();
    console.log('SimpleStorage deployed to:', await simpleStorage.getAddress());

    // Test the contract functions
    console.log('\nTesting contract functions...');
    
    // Test initial value
    const initialValue = await simpleStorage.get();
    console.log('Initial value:', initialValue);
    
    // Set a new value
    console.log('Setting value to 42...');
    const tx = await simpleStorage.set(42);
    await tx.wait();
    console.log('Transaction hash:', tx.hash);
    
    // Get the new value
    const newValue = await simpleStorage.get();
    console.log('New value:', newValue);
    
    // Check if test passed
    if (newValue === 42n) {
      console.log('\n✓ Test passed: Value was correctly set to 42');
    } else {
      console.log('\n✗ Test failed: Value was not set correctly');
    }
    
    console.log('\nTest completed successfully!');
  }
}

runTests().catch(console.error);